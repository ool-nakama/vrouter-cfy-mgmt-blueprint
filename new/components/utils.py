...
def copy_notice(service):
    ...
    if os.path.isfile(destn):
        ...
        notice_file = ctx.download_resouce('components/{0}/NOTICE.txt'.format(
            service))
        run('sudo mv {0} {1}'.format(notice_file, destn))
...
def start_systemd_service(service_name):
    run('sudo systemctl start {0}'.format(service_name))


def stop_systemd_service(service_name):
    run('sudo systemctl stop {0}'.format(service_name))
...
def get_selinux_state():
    return run('getenforce').aggr_stdout
